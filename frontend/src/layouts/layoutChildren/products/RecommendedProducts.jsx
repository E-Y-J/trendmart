...
import { getColdStart, searchRecommendations, getSimilarById } from "../../../api/recommendations";
...
function RecommendedProducts() {
  ...
  const [similarState, setSimilarState] = useState({ open: false, base: null, loading: false, error: null, items: [] });
    ...
    >
      ...
      {!searching && !state.loading && !state.error && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {state.items.map((p) => (
            <div key={p.id || p.product_id || p.external_id} className="border rounded-lg p-4 hover:shadow-sm transition">
              <div className="h-40 bg-gray-100 rounded mb-3 overflow-hidden flex items-center justify-center">
                {p.image_url ? (
                  <img src={p.image_url} alt={p.title || p.name} className="h-full object-cover" />
                ) : (
                  <div className="text-gray-400 text-sm">No image</div>
                )}
              </div>

              <div className="font-medium line-clamp-2">{p.title || p.name || "Untitled"}</div>
              {typeof p.price !== "undefined" && (
                <div className="text-sm text-gray-900 mt-1">${Number(p.price).toFixed(2)}</div>
              )}
              {p.description && (
              ...
              <div className="mt-3 flex gap-2">
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      await sendRecommendationFeedback({ product: p, action: "clicked", source });
                    } catch (e) {
                ...
                <button
                  ...
                  onClick={async () => {
                    setSimilarState({ open: true, base: p, loading: true, error: null, items: [] });
                    try {
                      const data = await getSimilarById(p.id || p.product_id || p.external_id, 6);
                      const items = Array.isArray(data?.results) ? data.results : data;
                      setSimilarState({ open: true, base: p, loading: false, error: null, items });
                    } catch (err) {
                      setSimilarState({ open: true, base: p, loading: false, error: err?.message || "Failed to load similar", items: [] });
                    }
                  }}
                >
                  Similar
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      await sendRecommendationFeedback({ product: p, action: "converted", source });
                    } catch (e) {
        ...
        <PopupLayout>
          <div style={{ padding: '1rem', minWidth: '60vw' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '.5rem' }}>
              <h3 style={{ margin: 0 }}>
                Similar to: {similarState.base?.title || similarState.base?.name}
              </h3>
              <button
                onClick={() => setSimilarState({ open: false, base: null, loading: false, error: null, items: [] })}
                style={{ padding: '.25rem .5rem', borderRadius: 4, border: '1px solid #ccc' }}
              >
                Close
              </button>
            </div>
            {similarState.loading && <div>Loading similar products…</div>}
            {similarState.error && <div style={{ color: 'red' }}>{similarState.error}</div>}
            {!similarState.loading && !similarState.error && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {similarState.items.map((p) => (
                  <div key={p.id || p.product_id || p.external_id} className="border rounded-lg p-4">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            console.debug("log view/feedback failed (similar)", e);
                          }
                        }}
                      >
                        View
                      ...
                      <button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            console.debug("log cart/feedback failed (similar)", e);
                          }
                        }}
                      >
                        Add to Cart
